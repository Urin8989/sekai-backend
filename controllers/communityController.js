...
exports.joinCommunity = async (req, res) => {
    try {
        ...
        if (community.participants.some(pId => pId.equals(userId))) return res.status(400).json({ message: 'You are already a member' });
        if (community.participants.length >= community.participantsLimit) return res.status(400).json({ message: 'Community is full' });
        ...
        // ★★★ WebSocketサーバーに参加者更新を通知 ★★★
        const wss = req.app.get('wss');
        if (wss && typeof wss.broadcastParticipantUpdate === 'function') {
...
exports.leaveCommunity = async (req, res) => {
    try {
        ...
        if (!community) return res.status(404).json({ message: 'Community not found' });
        if (community.organizer.equals(userId)) return res.status(400).json({ message: 'Organizer cannot leave.' });

        const initialLength = community.participants.length;
        community.participants = community.participants.filter(pId => !pId.equals(userId));

        if (community.participants.length === initialLength) return res.status(400).json({ message: 'You are not a member.' });
...
exports.deleteCommunity = async (req, res) => {
    try {
        ...
        if (!community) return res.status(404).json({ message: 'Community not found' });
        if (!community.organizer.equals(userId)) return res.status(403).json({ message: 'Only the organizer can delete.' });

        await CommunityChatMessage.deleteMany({ communityId: communityId });
        await Community.findByIdAndDelete(communityId);

        // ★★★ WebSocketサーバーにコミュニティ削除を通知 ★★★
        const wss = req.app.get('wss');
        if (wss && typeof wss.broadcastCommunityDeleted === 'function') {
            wss.broadcastCommunityDeleted(communityId);
        }
        // ★★★ 通知ここまで ★★★

        console.log(`Community ${community.name} deleted by ${req.user.name}`);
...
exports.getCommunityParticipants = async (req, res) => {
    try {
        ...
        const participants = community.participants.map(p => ({
            sub: p.googleId, name: p.name, picture: p.picture,
        }));
...
exports.getChatMessages = async (req, res) => {
    try {
        ...
        if (!community) return res.status(404).json({ message: 'Community not found' });
        if (!community.participants.some(id => id.equals(userId))) return res.status(403).json({ message: 'You are not a member' });

        const messages = await CommunityChatMessage.find({ communityId: communityId })
            .sort({ timestamp: -1 }).limit(50).populate('sender', 'googleId name picture');

        const formattedMessages = messages.reverse().map(msg => ({
            id: msg._id, communityId: msg.communityId, senderId: msg.sender?.googleId,
            senderName: msg.sender?.name || '不明なユーザー', senderPicture: msg.sender?.picture,
            text: msg.text, timestamp: msg.timestamp, type: 'community_chat_message'
        }));
...
exports.getCommunityById = async (req, res) => {
    try {
        ...
        const responseCommunity = {
            id: community._id, name: community.name, description: community.description,
            organizerId: community.organizer?._id, organizerGoogleId: community.organizer?.googleId,
            organizerName: community.organizer?.name || '不明', organizerPicture: community.organizer?.picture,
            participantsLimit: community.participantsLimit, currentParticipants: community.participants.length,
            joinPoints: community.joinPoints, createdAt: community.createdAt,
        };
...
exports.kickParticipant = async (req, res) => {
    try {
        const communityId = req.params.communityId;
        const { participantId: participantSubToKick } = req.body;
        const organizerSub = req.user.googleId;

        if (!participantSubToKick) return res.status(400).json({ message: '追放する参加者のIDを指定してください。' });

        const community = await Community.findById(communityId).populate('organizer', 'googleId');
        if (!community) return res.status(404).json({ message: 'コミュニティが見つかりません。' });
        if (!community.organizer || community.organizer.googleId !== organizerSub) return res.status(403).json({ message: '主催者のみが追放できます。' });
        if (participantSubToKick === organizerSub) return res.status(400).json({ message: '主催者は自身を追放できません。' });

        const userToKick = await User.findOne({ googleId: participantSubToKick });
        if (!userToKick) return res.status(404).json({ message: '追放対象の参加者が見つかりません。' });
        ...
        if (participantIndex === -1) return res.status(404).json({ message: '対象の参加者はメンバーではありません。' });

        community.participants.splice(participantIndex, 1);
        await community.save();

        // ★★★ WebSocketサーバーにキックを通知 ★★★
        const wss = req.app.get('wss');
        if (wss && typeof wss.kickUser === 'function') {
            wss.kickUser(communityId, participantSubToKick); // participantSubToKick は googleId (sub)
        } else {
            console.warn("WebSocket server or kickUser function not found. Cannot send kick notification.");
        }
        // ★★★ 通知ここまで ★★★

        console.log(`Participant ${userToKick.name} kicked from ${community.name}`);
        res.status(200).json({
            message: `${userToKick.name}さんを追放しました。`,
            currentParticipants: community.participants.length
        });
